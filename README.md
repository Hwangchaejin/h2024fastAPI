...
### 느낀점 및 배운 점
평상시에 나는 축구를 즐겨 보기도 한다. 그래서 이 주제를 선택 했을 때 흥미를 가지고 시작했다.
하지만 막상 데이터를 가지고 분석을 해보려니 쉽지 않았다.
잉글랜드 프리미어 리그 팀들에 대한 리뷰가 21만건이 떴었는데 생각보다 굉장히 많았었고
그걸 줄이기 위해서 런던 쪽 지역이 아닌 다른 지역들은 모두 제외 시켰다.
파이썬 프로그램을 돌려도 맞아 떨어지지 않는 결과값들이 수출되어서 이대로 안되나 싶었지만
여러번 시도 끝에 잘 완성된거 같다. 
이번 프로그램에서는 선수들의 몸값이나 팀들의 우승이 아닌 서포터즈 팬들의 반응을 알아볼 수 있는 계기가 되었기에
다양한 분석을 가질 수 있었고 특히 축구를 좋아하는 친구들에게도 쉽게 표를 보여주면서 설명해줄 수 있다고 생각한다.
또한 시간을 더 가질 수 있다면 더 정확한 데이터를 가지고 와서 구축해보면 큰 도움이 될 것 같다.
